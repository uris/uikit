...
export function RadioButtonList(props: RadioButtonListProps) {
  ...
  useEffect(() => {
    ...
    selectedOptions.forEach((item: string) => {
      options.forEach((option: RadioButtonOption, i: number) => {
  ...
  function isSelected(index: number): boolean {
    ...
    let exists = selected.indexOf(index) !== -1;
    return exists;
  }
  ...
  function handleChange(selection: number, state: boolean) {
    ...
    if (!multiSelect) {
      ...
      indexesSelected?.forEach((index: number) => {
